...
def extract_item_details(tree, file_name):
    ...
    for det in dets:
        ...
        vserv = vprod  # NFCom, podemos considerar vProd como valor de serviço
...
def process_single_xml_bytes(
    ...
    return erros_total, itens_detalhe, corrigido_bytes, foi_corrigido
...
def main():
    ...
    if uploaded and st.button("Validar arquivos"):
                            ...
                            for info in zf.infolist():
                                if info.filename.lower().endswith(".xml"):
                                    try:
                                        ...
                                        er, det, corr_bytes, foi_corrigido = process_single_xml_bytes(
                                            xml_bytes,
                                            logical_name,
                                            rules,
                                            cclass_cfg,
        ...
        # ZIP com TODOS os XMLs válidos (corrigidos e sem correção)
        if xml_resultados:
            buf = io.BytesIO()
        ...
        if xml_resultados:
            ...
            for x in xml_resultados:
                ...
                rows_status.append({"arquivo_base": base, "status": status})
            df_status_xml = pd.DataFrame(rows_status)
        else:
            df_status_xml = pd.DataFrame()

        # Resumo
        resumo_rows = []
        resumo_rows.append({
            "Métrica": "XMLs válidos processados",
            "Valor": len(xml_resultados)
        })
        resumo_rows.append({
            "Métrica": "XMLs corrigidos",
            "Valor": sum(1 for x in xml_resultados if x["corrigido"])
        })
        resumo_rows.append({
            "Métrica": "XMLs sem correção",
            "Valor": sum(1 for x in xml_resultados if not x["corrigido"])
        })
        resumo_rows.append({
            "Métrica": "Total de erros/alertas encontrados",
            "Valor": len(erros_total)
        })
        resumo_rows.append({
            "Métrica": "Total de itens de faturamento",
            "Valor": len(df_itens)
        })
        resumo_rows.append({
            "Métrica": "Total faturado (vServ)",
