...
# Caminho do logo da Contare no repositório (ajuste se necessário)
LOGO_PATH = "logo_contare.png"  # coloque esse arquivo na mesma pasta do app.py
...
def validate_with_rules_yaml(
    ...
    Ajuste:
    - As regras de PIS/COFINS por item (R_DET_PIS_VPIS_OBRIG e R_DET_COFINS_VCOFINS_OBRIG)
      só são aplicadas quando o emitente estiver no Regime Normal (CRT = 3).
    """
    all_errors: List[Dict[str, Any]] = []
    root = tree.getroot()
    ns = get_ns(tree)

    # Descobre o CRT do emitente (regime tributário)
    if ns:
    ...
    for rule in rules:
        ...
        # Pula regras de PIS/COFINS por item quando não for Regime Normal
...
def validate_cfop_pf_pj_nao_contrib(
    ...
    """
    PF/PJ não contribuinte: se destinatário for PF ou PJ não contribuinte de ICMS,
    então, para itens que NÃO são SVA, o CFOP deve ser:
       5307 (operações internas) ou
       6307 (operações interestaduais).

    Itens SVA são ignorados aqui.
    Se o CFOP estiver em branco, NÃO consideramos erro.
    """
    erros: List[Dict[str, Any]] = []
    ...
    for det in det_nodes:
        ...
        # Se CFOP estiver vazio, não vamos considerar erro aqui
        if not cfop:
            continue

        # Só valida se houver CFOP preenchido e ele for diferente dos permitidos
...
def validate_sva_cfop_zero(
    ...
    """
    Se o cClass do item estiver na lista de SVA, o CFOP deve estar "zerado":
      - Sem CFOP informado (OK), ou
      - Igual ao valor configurado cfop_sva_zerado (default "0000").
    Qualquer outro CFOP é erro.
    """
    erros: List[Dict[str, Any]] = []
...
def validate_custom_rules(
    ...
    # CFOP para PF/PJ não contribuinte (ignorando SVA e CFOP vazio)
...
def extract_faturamento_items(tree: etree._ElementTree, file_name: str) -> List[Dict[str, Any]]:
    ...
    Para NFCom, usa:
      ...
      - xProd    em det/prod/xProd  (descrição)
    ...
    for det in det_nodes:
        ...
        descricao = (xprod_nodes[0].text or "").strip() if xprod_nodes else ""
...
# =========================
# Função de correção: remover CFOP de itens SVA
# =========================

def fix_cfop_sva_in_xml(tree: etree._ElementTree, cclass_cfg: Dict[str, Any]) -> bytes:
    """
    Gera um novo XML (em bytes) a partir do XML original,
    removendo o CFOP dos itens cujo cClass esteja na lista de SVA.

    Não altera o tree original; trabalha em uma cópia.
    """
    root_orig = tree.getroot()
    # Clona a árvore inteira para não impactar validações
    root_copy = etree.fromstring(etree.tostring(root_orig))
    ...
    if not sva_cclasses:
        return etree.tostring(tree_copy, encoding="utf-8", xml_declaration=True)

    det_nodes = root_copy.xpath(".//n:det", namespaces=ns) if ns else root_copy.xpath(".//det")
    ...
    for det in det_nodes:
        ...
        if cclass_text in sva_cclasses and cfop_nodes:
            for cfop in cfop_nodes:
                parent = cfop.getparent()
...
def generate_pdf_report(df_erros: pd.DataFrame, logo_path: str = LOGO_PATH) -> io.BytesIO:
    """
    Gera um PDF em memória contendo o relatório de erros da NFCom,
    incluindo logo da Contare e assinatura do desenvolvedor.
    """
    buffer = io.BytesIO()
    ...
    # Cabeçalho com logo
    try:
        c.drawImage(logo_path, x_margin, height - 90, width=80, preserveAspectRatio=True, mask='auto')
        c.setFont("Helvetica-Bold", 14)
        c.drawString(x_margin + 90, height - 60, "Relatório de Erros – NFCom Modelo 62")
        y = height - 110
    except Exception:
        # Se não encontrar o logo, apenas título
        c.setFont("Helvetica-Bold", 14)
        c.drawString(x_margin, y, "Relatório de Erros – NFCom Modelo 62")
        y -= 40

    c.setFont("Helvetica", 10)

    # garante colunas
    for col in ["arquivo", "regra_id", "descricao_regra", "campo_xpath",
                "valor_encontrado", "mensagem_erro", "sugestao_correcao"]:
    ...
    for _, row in df_erros.iterrows():
        bloco = [
            ...
            f"Campo: {row['campo_xpath']}",
            f"Valor encontrado: {row['valor_encontrado']}",
            f"Erro: {row['mensagem_erro']}",
            f"Correção sugerida: {row['sugestao_correcao']}",
            "-" * 110
        ]

        for linha in bloco:
            wrapped = simpleSplit(str(linha), "Helvetica", 10, width - 2 * x_margin)
            for text_line in wrapped:
                if y < 70:
                    # Rodapé ao trocar de página
                    ...
                    c.setFont("Helvetica", 10)
                    y = height - 60
                c.drawString(x_margin, y, text_line)
                y -= 14
...
def main():
    ...
    with col_title:
        st.markdown(
            "<h2 style='margin-bottom:0'>Validador de NFCom modelo 62</h2>"
            "<p style='margin-top:4px;'>Contare – Contabilidade especializada em Provedores de Internet</p>",
            unsafe_allow_html=True,
        )

    st.write(
        "Faça o upload de vários arquivos XML de NFCom e gere relatórios de "
        "erros de parametrização/tributação, faturamento por CClass e XMLs corrigidos para importação."
    )
    ...
    if uploaded_files and st.button("Validar arquivos"):
        ...
        xml_corrigidos: List[Dict[str, Any]] = []
        ...
        for f in uploaded_files:
            ...
            try:
                ...
                # Geração de XML corrigido removendo CFOP de SVA
                xml_corrigido_bytes = fix_cfop_sva_in_xml(tree, cclass_cfg)
                nome_corrigido = file_name.replace(".xml", "_corrigido.xml")
                xml_corrigidos.append({
                    "nome": nome_corrigido,
                    "conteudo": xml_corrigido_bytes,
                })
        ...
        # Botão para baixar XMLs corrigidos (CFOP removido em SVA)
        if xml_corrigidos:
            zip_buffer = io.BytesIO()
            with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
                for item in xml_corrigidos:
                    zf.writestr(item["nome"], item["conteudo"])
            zip_buffer.seek(0)

            st.download_button(
                "Baixar XMLs corrigidos (CFOP removido de SVAs)",
                data=zip_buffer,
                file_name="xml_corrigidos_sva_sem_cfop.zip",
        ...
        if not todos_erros:
            ...
            # Garante colunas de mensagem/sugestão
            for col in ["mensagem_erro", "sugestao_correcao"]:
                if col not in df_erros.columns:
                    df_erros[col] = ""

            # Coluna unificada de instruções
            df_erros["instrucoes_correcao"] = (
                df_erros["mensagem_erro"].fillna("") + " " +
                df_erros["sugestao_correcao"].fillna("")
            ).str.strip()
            ...
            with tab1:
                ...
                # PDF
                pdf_buffer = generate_pdf_report(df_erros, logo_path=LOGO_PATH)
                st.download_button(
            ...
            with tab4:
                ...
                colm1.metric("Total de ocorrências de erros/alertas", total_erros)
                colm2.metric("Arquivos com problemas", total_arquivos)
                ...
                if not by_regra.empty:
                    st.dataframe(by_regra, use_container_width=True)
                    st.bar_chart(by_regra.set_index("regra_id")["qtd_erros"])
                ...
                if not by_arquivo.empty:
                    st.dataframe(by_arquivo, use_container_width=True)
                    st.bar_chart(by_arquivo.set_index("arquivo")["qtd_erros"])

                st.markdown("### Erros por nível (erro/alerta)")
                by_nivel = (
                    df_erros
                    .groupby("nivel")
                    .size()
                ...
                if not by_nivel.empty:
                    st.dataframe(by_nivel, use_container_width=True)
                    st.bar_chart(by_nivel.set_index("nivel")["qtd"])
    ...
    # Rodapé do app
    st.markdown(
        "<hr><p style='text-align:center; font-size:0.8rem;'>"
        "Desenvolvido por <b>Raul Martins</b> – Contare Contabilidade especializada em Provedores de Internet"
        "</p>",
        unsafe_allow_html=True,
    )
