...
def process_single_xml_bytes(xml_bytes: bytes, logical_name: str, rules, cclass_cfg, corrigir_descontos: bool):
    """
    ...
    # Faturamento
    faturamento.extend(extract_faturamento_items(tree, logical_name))
    # XML corrigido
    corrigido_bytes = generate_corrected_xml(tree, cclass_cfg, corrigir_descontos)

    return erros_total, faturamento, corrigido_bytes
...
def main():
    ...
    st.write("Valide, corrija e gere relatórios completos de NFCom (modelo 62).")
    ...
    if uploaded and st.button("Validar arquivos"):
        ...
        for f in uploaded:
            ...
            try:
                ...
                if nome.lower().endswith(".zip"):
                    # Processar cada XML dentro do ZIP
                    try:
                            ...
                            for info in zf.infolist():
                                if info.filename.lower().endswith(".xml"):
                                    try:
                                        ...
                                        er, fat, corr = process_single_xml_bytes(
                                            xml_bytes,
                                            logical_name,
                                            rules,
                                            cclass_cfg,
                                        ...
                                        faturamento.extend(fat)
                                        xml_corrigidos.append({
                                            "nome": info.filename.replace(".xml", "_corrigido.xml"),
                                            "conteudo": corr
                                        })
                    ...
                    faturamento.extend(fat)
                    xml_corrigidos.append({
                        "nome": nome.replace(".xml", "_corrigido.xml"),
                        "conteudo": corr
                    })
        ...
        if xml_corrigidos:
            ...
            st.download_button(
                ...
                data=buf,
                file_name="xml_corrigidos_nfcom.zip",
                mime="application/zip"
            )

        # Se não houver faturamento nem erros
        if not erros_total and not faturamento:
            st.warning("Nenhum XML NFCom válido foi encontrado nos arquivos enviados.")
            return

        # ---------------------------
        # Se houver erros -> abas
        # ---------------------------
        if erros_total:
            ...
            with tab2:
                if consolidar:
                    ...
                    st.download_button(
                        ...
                        agrupado.to_csv(index=False),
                        "erros_consolidados.csv"
                    )
                else:
                    st.info("Marque 'Consolidar erros iguais' na barra lateral para agrupar.")

            # Faturamento (quando houver erros) – compartilhado com aba abaixo
            with tab3:
                st.subheader("Relatório de faturamento por CClass e Item")
                if faturamento:
                    df_f = pd.DataFrame(faturamento)

                    # Totais por item + cClass
                    df_item_cclass = (
                    ...
                    total_geral = df_cclass["total_vServ"].sum()
                    if total_geral > 0:
                        df_cclass["participacao_%"] = (df_cclass["total_vServ"] / total_geral) * 100
            ...
            with tab4:
                ...
                total_regras = df["regra_id"].nunique()
        ...
        # mesmo sem erros
        # ---------------------------
        st.subheader("Relatório de faturamento por CClass e Item (sempre disponível)")
        if faturamento:
            df_f = pd.DataFrame(faturamento)
