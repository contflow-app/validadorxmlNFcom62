...
def process_single_xml_bytes(
    ...
    return erros_total, faturamento, corrigido_bytes, foi_corrigido
...
def main():
    ...
    if uploaded and st.button("Validar arquivos"):
        ...
        xml_resultados: List[Dict[str, Any]] = []  # {base_name, conteudo, corrigido}
                            ...
                            for info in zf.infolist():
                                if info.filename.lower().endswith(".xml"):
                                    try:
                                        ...
                                        # Nome lógico para mensagens: zip::arquivo interno
                                        logical_name = f"{nome}::{info.filename}"
                                        # Nome base flatten para ZIP de saída (pasta1/arquivo.xml → pasta1_arquivo.xml)
                                        base_name = info.filename.replace("\\", "/").replace("/", "_")

                                        er, fat, corr_bytes, foi_corrigido = process_single_xml_bytes(
                                            xml_bytes,
                                            logical_name,
                                            rules,
                                            cclass_cfg,
        ...
        if xml_resultados:
            ...
            buf.seek(0)
            st.download_button(
                "Baixar XMLs (corrigidos e sem correção)",
                data=buf,
                file_name="xml_nfcom_processados.zip",
