...
def validate_with_rules_yaml(
    ...
    for rule in rules:
        ...
        # Regras condicionais (depende de outro campo)
        if rule_type == "condicional":
            ...
            for base_node in base_nodes:
                ...
                if cond_values and cond_text not in cond_values:
                    continue  # condição não satisfeita → não valida essa regra
                ...
                if alvo_valor_esperado is not None:
                    ...
                    # Apenas checagem de preenchimento / não preenchimento
        ...
        # Tipos customizados tratados em Python ficam de fora aqui
    ...
    for det in det_nodes:
        ...
        if cfop_text != cfop_esperado:
            erros.append({
                ...
                "descricao_regra": "Se cClass for SVA, CFOP não deve estar tributário (deve estar 'zerado')",
                "campo_xpath": ".//det/prod/CFOP",
...
def extract_faturamento_items(tree: etree._ElementTree, file_name: str) -> List[Dict[str, Any]]:
    ...
    for det in det_nodes:
        if ns:
            ...
            # tenta ICMS/vICMS → se não houver, deixa 0
