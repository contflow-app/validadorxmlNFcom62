...
def validate_with_rules_yaml(
    ...
    - As regras de PIS/COFINS por item (R_DET_PIS_VPIS_OBRIG e R_DET_COFINS_VCOFINS_OBRIG)
      só serão aplicadas quando o emitente estiver no Regime Normal (CRT = 3).
      Para Simples Nacional (CRT = 1 ou 2), esses campos não são obrigatórios por item.
    """
...
def validate_cfop_pf_pj_nao_contrib(
    ...
    Se o CFOP estiver em branco, NÃO vamos considerar erro aqui.
    Só apontamos erro quando houver CFOP preenchido e ele for diferente do esperado.
    """
    erros: List[Dict[str, Any]] = []
...
def generate_pdf_report(df_erros: pd.DataFrame) -> io.BytesIO:
    """
    Gera um PDF em memória contendo o relatório de erros da NFCom,
    incluindo mensagem e instruções de correção.
    """
    buffer = io.BytesIO()
    ...
    if uploaded_files and st.button("Validar arquivos"):
        ...
        if not todos_erros:
            ...
            with tab1:
                ...
                # PDF
                pdf_buffer = generate_pdf_report(df_erros)
                st.download_button(
